...
export function ActivityPage({ onOpenPayment }: { onOpenPayment: (id: string) => void }) {
  ...
  return <>
    ...
    <section className="filter-bar"><div className="search-box"><span aria-hidden="true">⌕</span><input aria-label="Search activity" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search activity…"/></div><select aria-label="Filter activity by type" value={kind} onChange={(e) => setKind(e.target.value)}><option value="">All activity</option><option value="payment">Payment changes</option><option value="payment_detected">Incoming payments</option><option value="webhook">Webhooks</option></select></section>
