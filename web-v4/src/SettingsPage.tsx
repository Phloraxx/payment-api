...
function ProfileModal({ profile, onClose, onSaved }: { profile?: Profile; onClose: () => void; onSaved: () => Promise<void> }) {
  const [id, setId] = useState(profile?.id ?? ""); const [label, setLabel] = useState(profile?.label ?? ""); const [upi, setUpi] = useState(profile?.upi_id ?? ""); const [payee, setPayee] = useState(profile?.payee_name ?? ""); const parser = profile?.parser ?? "paytm_notification"; const [enabled, setEnabled] = useState(profile?.enabled ?? true); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
