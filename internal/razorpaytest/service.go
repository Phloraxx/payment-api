...
func (s *Service) Create(ctx context.Context, input CreateInput) (*core.Record, bool, error) {
	...
	if input.AmountPaise < 100 || input.AmountPaise > 100_000_00 {
		return nil, false, domain.New("RAZORPAY_TEST_INVALID_AMOUNT", "test amount must be between ₹1 and ₹1,00,000", 400)
	}
	if input.IdempotencyKey == "" || len(input.IdempotencyKey) > 255 {
		return nil, false, domain.New("RAZORPAY_TEST_IDEMPOTENCY_REQUIRED", "a valid Idempotency-Key is required", 400)
	}
	if len(input.ExternalID) > 255 {
		return nil, false, domain.InvalidExternalID()
	}

	if existing, err := s.App.FindFirstRecordByData("razorpay_test_orders", "idempotency_key", input.IdempotencyKey); err == nil {
		if int64(existing.GetInt("amount")) != input.AmountPaise || existing.GetString("external_id") != input.ExternalID {
			return nil, false, domain.New("RAZORPAY_TEST_IDEMPOTENCY_CONFLICT", "the idempotency key was already used with different parameters", 409)
		}
		if status := existing.GetString("status"); status == "creating" || status == "create_failed" {
			domainErr := domain.New("RAZORPAY_TEST_CREATE_STATE_UNKNOWN", "the previous provider-order attempt did not complete cleanly; inspect the Razorpay Test Dashboard using the local receipt before starting a new attempt", 409)
	...
	collection, err := s.App.FindCollectionByNameOrId("razorpay_test_orders")
	if err != nil {
		return nil, false, err
	}
	now := s.now()
	...
	if err := s.App.Save(record); err != nil {
		if existing, findErr := s.App.FindFirstRecordByData("razorpay_test_orders", "idempotency_key", input.IdempotencyKey); findErr == nil {
			return existing, true, nil
		}
		return nil, false, err
	}

	providerOrder, err := s.Client.CreateOrder(ctx, input.AmountPaise, "pgt_"+record.Id)
	if err != nil {
		record.Set("status", "create_failed")
		record.Set("error", truncate(err.Error(), 4096))
		record.Set("last_synced_at", now)
		_ = s.App.Save(record)
		domainErr := domain.New("RAZORPAY_TEST_CREATE_FAILED", "Razorpay test order creation failed", 502)
...
func (s *Service) Get(localOrderID string) (*core.Record, error) {
	record, err := s.App.FindRecordById("razorpay_test_orders", strings.TrimSpace(localOrderID))
	if errors.Is(err, sql.ErrNoRows) {
		return nil, domain.New("RAZORPAY_TEST_ORDER_NOT_FOUND", "Razorpay test order not found", 404)
	}
	return record, err
}
...
func (s *Service) Verify(ctx context.Context, input VerifyInput) (*core.Record, error) {
	...
	if providerOrderID == "" || input.RazorpayOrderID != providerOrderID {
		return nil, domain.New("RAZORPAY_TEST_ORDER_MISMATCH", "checkout order id does not match the server-created order", 400)
	}
	if !strings.HasPrefix(input.RazorpayPaymentID, "pay_") {
		return nil, domain.New("RAZORPAY_TEST_INVALID_PAYMENT", "invalid Razorpay payment id", 400)
	}
	if !verifyHexHMAC(s.KeySecret, providerOrderID+"|"+input.RazorpayPaymentID, input.RazorpaySignature) {
		return nil, domain.New("RAZORPAY_TEST_SIGNATURE_INVALID", "Razorpay checkout signature verification failed", 400)
	}

	err = s.App.RunInTransaction(func(tx core.App) error {
		current, err := tx.FindRecordById("razorpay_test_orders", record.Id)
		if err != nil {
			return err
		}
		if existing := current.GetString("razorpay_payment_id"); existing != "" && existing != input.RazorpayPaymentID {
			return domain.New("RAZORPAY_TEST_PAYMENT_CONFLICT", "the order is already linked to another Razorpay payment", 409)
		}
		if other, findErr := tx.FindFirstRecordByData("razorpay_test_orders", "razorpay_payment_id", input.RazorpayPaymentID); findErr == nil && other.Id != current.Id {
			return domain.New("RAZORPAY_TEST_PAYMENT_CONFLICT", "the Razorpay payment is already linked to another test order", 409)
	...
	if _, refreshErr := s.Refresh(ctx, record.Id); refreshErr != nil {
		...
		if errors.As(refreshErr, &domainErr) && domainErr.Code != "RAZORPAY_TEST_REFRESH_FAILED" {
...
func (s *Service) Refresh(ctx context.Context, localOrderID string) (*core.Record, error) {
	...
	if paymentID == "" {
		return nil, domain.New("RAZORPAY_TEST_PAYMENT_UNKNOWN", "no Razorpay payment id is linked to this order yet", 409)
	}
	payment, err := s.Client.FetchPayment(ctx, paymentID)
	if err != nil {
		return nil, domain.New("RAZORPAY_TEST_REFRESH_FAILED", "could not fetch the Razorpay payment", 502)
	}
...
func (s *Service) IngestWebhook(eventID, signature string, raw []byte) (WebhookResult, error) {
	...
	if eventID == "" || len(eventID) > 128 {
		return WebhookResult{}, domain.New("RAZORPAY_TEST_EVENT_ID_REQUIRED", "X-Razorpay-Event-Id is required", 400)
	}
	if len(raw) == 0 || len(raw) > maxWebhookBytes {
		return WebhookResult{}, domain.New("RAZORPAY_TEST_WEBHOOK_INVALID", "webhook body must be between 1 byte and 1 MiB", 400)
	}
	if !verifyHexHMAC(s.WebhookSecret, string(raw), signature) {
		return WebhookResult{}, domain.New("RAZORPAY_TEST_WEBHOOK_SIGNATURE_INVALID", "invalid Razorpay webhook signature", 401)
	}
	hashBytes := sha256.Sum256(raw)
	payloadHash := hex.EncodeToString(hashBytes[:])
	if existing, err := s.App.FindFirstRecordByData("razorpay_test_events", "event_id", eventID); err == nil {
		if existing.GetString("payload_hash") != payloadHash {
			return WebhookResult{}, domain.New("RAZORPAY_TEST_EVENT_ID_CONFLICT", "the Razorpay event id was already used with a different payload", 409)
		}
		return WebhookResult{Duplicate: true, EventID: eventID, OrderID: existing.GetString("test_order"), Status: existing.GetString("status")}, nil
	} else if !errors.Is(err, sql.ErrNoRows) {
	...
	if err := json.Unmarshal(raw, &envelope); err != nil {
		return WebhookResult{}, domain.New("RAZORPAY_TEST_WEBHOOK_INVALID", "invalid Razorpay webhook JSON", 400)
	}
	payment := envelope.Payload.Payment.Entity
	result := WebhookResult{EventID: eventID}
	now := s.now()
	err := s.App.RunInTransaction(func(tx core.App) error {
		if existing, err := tx.FindFirstRecordByData("razorpay_test_events", "event_id", eventID); err == nil {
			if existing.GetString("payload_hash") != payloadHash {
				return domain.New("RAZORPAY_TEST_EVENT_ID_CONFLICT", "the Razorpay event id was already used with a different payload", 409)
			}
			result.Duplicate = true
			result.OrderID = existing.GetString("test_order")
			result.Status = existing.GetString("status")
			return nil
		} else if !errors.Is(err, sql.ErrNoRows) {
			return err
		}
		collection, err := tx.FindCollectionByNameOrId("razorpay_test_events")
		if err != nil {
			return err
		}
		...
		order, findErr := tx.FindFirstRecordByData("razorpay_test_orders", "razorpay_order_id", payment.OrderID)
		if errors.Is(findErr, sql.ErrNoRows) {
			event.Set("status", "ignored")
			event.Set("error", "No local Razorpay test order matches this event")
			result.Ignored = true
...
func (s *Service) applyPayment(localOrderID string, payment ProviderPayment, at time.Time) error {
	return s.App.RunInTransaction(func(tx core.App) error {
		order, err := tx.FindRecordById("razorpay_test_orders", localOrderID)
		if err != nil {
			return err
		}
		if err := validateProviderPayment(order, payment); err != nil {
...
func validateProviderPayment(order *core.Record, payment ProviderPayment) error {
	if payment.ID == "" || payment.OrderID != order.GetString("razorpay_order_id") {
		return domain.New("RAZORPAY_TEST_PROVIDER_MISMATCH", "Razorpay payment does not belong to the local order", 409)
	}
	if payment.Amount != int64(order.GetInt("amount")) || !strings.EqualFold(payment.Currency, order.GetString("currency")) {
		return domain.New("RAZORPAY_TEST_PROVIDER_MISMATCH", "Razorpay payment amount or currency does not match the local order", 409)
	}
	return nil
}
...
func applyProviderPayment(order *core.Record, payment ProviderPayment, at time.Time) error {
	if existing := order.GetString("razorpay_payment_id"); existing != "" && existing != payment.ID {
		return domain.New("RAZORPAY_TEST_PAYMENT_CONFLICT", "the local order is linked to another Razorpay payment", 409)
	}
