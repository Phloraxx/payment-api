...
func TestIngestMatchesPaytmNotificationByExactDDMAmount(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if result.Status != "matched" || result.Action != "marked_paid" || result.PaymentID != payment.ID { t.Fatalf("result = %+v", result) }
	stored, err := paymentService.Get(payment.ID)
	if err != nil || stored.Status != domain.StatusPaid || stored.PayerName != "Test User" || stored.EvidenceSource != "paytm_notification" { t.Fatalf("stored = %+v err=%v", stored, err) }
	event, err := app.FindRecordById("notification_events", result.EventID)
	if err != nil || event.GetString("processing_status") != "matched" || event.GetString("matched_payment") != payment.ID { t.Fatalf("event = %+v err=%v", event, err) }
}
...
func TestIngestDedupesAndRejectsWrongApp(t *testing.T) {
	...
	if err != nil || !second.Duplicate || second.Action != "duplicate_event" || second.EventID != first.EventID { t.Fatalf("duplicate = %+v err=%v", second, err) }
	if count, _ := app.CountRecords("notification_events"); count != 1 { t.Fatalf("notification count = %d; want 1", count) }
	if _, err := service.Ingest(Input{SourceEventID: "evt-wrong", AppPackage: "com.example.fake", Body: "₹1.01 paid by User"}); err == nil { t.Fatal("wrong app package was accepted") }
}
...
func TestDelayedOldNotificationCannotConfirmReusedDDMAmount(t *testing.T) {
	...
	*now = now.Add(24*time.Hour + 6*time.Minute + time.Second)
	...
	if err != nil { t.Fatal(err) }
	if first.PayablePaise != second.PayablePaise { t.Fatalf("test needs reused amount: %d vs %d", first.PayablePaise, second.PayablePaise) }
	...
	if err != nil { t.Fatal(err) }
	if result.Status != "unmatched" { t.Fatalf("old notification result = %+v", result) }
	stored, _ := paymentService.Get(second.ID)
	if stored.Status != domain.StatusPending { t.Fatalf("new payment incorrectly became %s", stored.Status) }
}
