...
func NewClient(keyID, keySecret, sourceTag string) *Client {
	return &Client{
		...
		BaseURL: productionAPIBaseURL, SourceTag: sourceTag,
	}
}
...
func (c *Client) CreateOrder(ctx context.Context, amountPaise int64, receipt string) (ProviderOrder, error) {
	payload := map[string]any{
		...
		"notes": map[string]string{"source": c.sourceTag()},
	}
...
func (c *Client) httpClient() *http.Client {
	...
	return NewClient(c.KeyID, c.KeySecret, c.SourceTag).HTTP
}

func (c *Client) sourceTag() string {
	if strings.TrimSpace(c.SourceTag) != "" {
		return strings.TrimSpace(c.SourceTag)
	}
	return "paygate_razorpay"
}
