...
func testService(t *testing.T) (*Service, *fakeProviderClient, *tests.TestApp, *time.Time) {
	...
	service := NewService(app, client, "rzp_live_key", "checkout-secret-123456", "webhook-secret-123456789012", "PayGate Live")
...
func TestCreateIsIdempotentAndDoesNotCreateProviderDuplicates(t *testing.T) {
	...
	_, _, err = service.Create(context.Background(), CreateInput{AmountPaise: 100, ExternalID: "order-2", IdempotencyKey: "idem-1"})
	var domainErr *domain.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_IDEMPOTENCY_CONFLICT" {
...
func TestVerifyRejectsTamperingAndOnlyCapturedProviderStateMarksPaid(t *testing.T) {
	...
	order, _, err := service.Create(context.Background(), CreateInput{AmountPaise: 100, IdempotencyKey: "idem-verify"})
	if err != nil {
		t.Fatal(err)
	}
	paymentID := "pay_test_123"
	client.payment = ProviderPayment{
		ID: paymentID, OrderID: order.GetString("razorpay_order_id"), Amount: 100,
	...
	var domainErr *domain.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_SIGNATURE_INVALID" {
...
func TestWebhookIsSignedDeduplicatedAndMonotonic(t *testing.T) {
	...
	order, _, err := service.Create(context.Background(), CreateInput{AmountPaise: 100, IdempotencyKey: "idem-webhook"})
	if err != nil {
		t.Fatal(err)
	}
	captured := webhookBody(t, "payment.captured", ProviderPayment{
		ID: "pay_webhook_1", OrderID: order.GetString("razorpay_order_id"), Amount: 100,
		Currency: "INR", Status: "captured", Captured: true, Method: "upi",
	})
	...
	stored, _ := app.FindRecordById("razorpay_live_orders", order.Id)
	...
	if count, _ := app.CountRecords("razorpay_live_events"); count != 2 {
...
func TestWebhookRejectsInvalidSignatureAndPersistsMismatchAsFailed(t *testing.T) {
	service, _, app, _ := testService(t)
	order, _, err := service.Create(context.Background(), CreateInput{AmountPaise: 100, IdempotencyKey: "idem-mismatch"})
	if err != nil {
		t.Fatal(err)
	}
	body := webhookBody(t, "payment.captured", ProviderPayment{
		ID: "pay_mismatch", OrderID: order.GetString("razorpay_order_id"), Amount: 101,
		Currency: "INR", Status: "captured",
	})
	_, err = service.IngestWebhook("evt_invalid_sig", "invalid", body)
	var domainErr *domain.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_WEBHOOK_SIGNATURE_INVALID" {
...
func TestCreateFailureCannotBeSilentlyRetriedWithSameIdempotencyKey(t *testing.T) {
	...
	var domainErr *domain.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_CREATE_FAILED" {
		t.Fatalf("first error=%v", err)
	}
	_, _, err = service.Create(context.Background(), CreateInput{AmountPaise: 100, IdempotencyKey: "idem-timeout"})
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_CREATE_STATE_UNKNOWN" {
...
func TestWebhookRejectsEventIDReusedWithDifferentPayload(t *testing.T) {
	...
	order, _, err := service.Create(context.Background(), CreateInput{AmountPaise: 100, IdempotencyKey: "idem-event-conflict"})
	if err != nil {
		t.Fatal(err)
	}
	first := webhookBody(t, "payment.captured", ProviderPayment{ID: "pay_conflict", OrderID: order.GetString("razorpay_order_id"), Amount: 100, Currency: "INR", Status: "captured"})
	if _, err := service.IngestWebhook("evt_same", Sign(service.WebhookSecret, first), first); err != nil {
		t.Fatal(err)
	}
	second := webhookBody(t, "payment.failed", ProviderPayment{ID: "pay_conflict", OrderID: order.GetString("razorpay_order_id"), Amount: 100, Currency: "INR", Status: "failed"})
	_, err = service.IngestWebhook("evt_same", Sign(service.WebhookSecret, second), second)
	var domainErr *domain.Error
	if !errors.As(err, &domainErr) || domainErr.Code != "RAZORPAY_LIVE_EVENT_ID_CONFLICT" {
