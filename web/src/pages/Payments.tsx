  ...
  return <div className="payments-layout">
    <section className="payments-toolbar panel">
      <div className="payment-search">
        ...
        <input value={query} onChange={(event) => { setQuery(event.target.value); setOffset(0); }} placeholder="Search payment, customer, order, UPI or reference" aria-label="Search payments" />
      ...
      <div className="filter-bar">
        ...
        <label className="filter-select"><span>Sort</span><select value={sort} onChange={(event) => { setSort(event.target.value); setOffset(0); }}><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="amount_desc">Amount ↓</option><option value="amount_asc">Amount ↑</option></select></label>
        {(status || account || query) && <button className="clear-filters" onClick={() => { setStatus(""); setAccount(""); setQuery(""); setOffset(0); }}>Clear filters</button>}
      </div>
    </section>
...
function PaymentOverview({ payment }: { payment: OperatorPaymentDetail }) {
  return <div className="drawer-section-stack">
    <DetailSection title="Customer & purpose"><Detail label="Display name">{payment.displayName || "—"}</Detail><Detail label="Order / external ID">{payment.externalId || "—"}</Detail><Detail label="Customer">{payment.customerName || "—"}</Detail><Detail label="Email">{payment.customerEmail || "—"}</Detail><Detail label="Phone">{payment.customerPhone || "—"}</Detail>{payment.description && <p className="detail-paragraph">{payment.description}</p>}</DetailSection>
...
function FinancialTruth({ payment }: { payment: OperatorPaymentDetail }) {
  return <div className="drawer-section-stack"><div className="protected-banner"><span>Protected financial record</span><p>These fields drive matching, uniqueness and audit guarantees. They cannot be silently edited.</p></div><DetailSection title="Payment"><Detail label="Requested amount">{money(payment.requestedAmountPaise)}</Detail><Detail label="Payable amount">{money(payment.payableAmountPaise)}</Detail><Detail label="Account">{payment.paymentAccount}</Detail><Detail label="Status"><Badge status={payment.status} /></Detail></DetailSection><DetailSection title="Evidence"><Detail label="RRN / UTR">{payment.rrn || "—"}</Detail><Detail label="Evidence reference">{payment.evidenceReference || "—"}</Detail><Detail label="Evidence source">{payment.evidenceSource || "—"}</Detail><Detail label="Detected payer">{payment.payerName || "—"}</Detail><Detail label="Detected UPI ID">{payment.upiId || "—"}</Detail></DetailSection><DetailSection title="Creation identity"><Detail label="External ID">{payment.externalId || "—"}</Detail><Detail label="Original metadata"><code>{JSON.stringify(payment.metadata || {})}</code></Detail><Detail label="Idempotency key"><code>{payment.idempotencyKey || "—"}</code></Detail></DetailSection><DetailSection title="Integrity"><Detail label="Reuse after">{formatDate(payment.reuseAfter)}</Detail><Detail label="Payment ID"><code>{payment.id}</code></Detail></DetailSection></div>;
}
...
function Detail({ label, children }: { label: string; children: ReactNode }) { return <div className="detail-row"><dt>{label}</dt><dd>{children}</dd></div>; }
