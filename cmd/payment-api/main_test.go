...
func TestMergeManagedRateLimitRulesIsIdempotentAndPreservesCustomRules(t *testing.T) {
	...
	for _, label := range []string{"POST /api/events/sms", "POST /api/events/paytm-notification", "POST /api/events/email", "POST /api/webhook", "POST /api/payments", "POST /api/razorpay/test/orders", "POST /api/razorpay/test/webhook", "custom"} {
		if counts[label] != 1 {
			t.Fatalf("label %s count=%d", label, counts[label])
		}
	}
	if second[0].MaxRequests != 60 || second[4].MaxRequests != 120 {
