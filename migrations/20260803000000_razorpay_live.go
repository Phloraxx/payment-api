...
func init() {
	pbmigrations.Register(func(app core.App) error {
		...
		orders, err := findOrCreateRazorpayLiveOrders(app, users.Id)
		if err != nil {
			return err
		}
		_, err = findOrCreateRazorpayLiveEvents(app, orders.Id)
		return err
	}, func(app core.App) error {
		for _, name := range []string{"razorpay_live_events", "razorpay_live_orders"} {
...
func findOrCreateRazorpayLiveOrders(app core.App, usersID string) (*core.Collection, error) {
	if collection, err := app.FindCollectionByNameOrId("razorpay_live_orders"); err == nil {
		return collection, nil
	}
	collection := core.NewBaseCollection("razorpay_live_orders")
	...
	collection.AddIndex("idx_rzp_live_idempotency", true, "idempotency_key", "")
	collection.AddIndex("idx_rzp_live_order", true, "razorpay_order_id", "razorpay_order_id != ''")
	collection.AddIndex("idx_rzp_live_payment", true, "razorpay_payment_id", "razorpay_payment_id != ''")
	collection.AddIndex("idx_rzp_live_status", false, "status,created_at", "")
...
func findOrCreateRazorpayLiveEvents(app core.App, ordersID string) (*core.Collection, error) {
	if collection, err := app.FindCollectionByNameOrId("razorpay_live_events"); err == nil {
		return collection, nil
	}
	collection := core.NewBaseCollection("razorpay_live_events")
	lockDomainWrites(collection)
	collection.Fields.Add(
		&core.TextField{Name: "event_id", Max: 128, Required: true},
		&core.TextField{Name: "event_type", Max: 128, Required: true},
		&core.RelationField{Name: "live_order", CollectionId: ordersID, MaxSelect: 1},
		&core.TextField{Name: "razorpay_order_id", Max: 64},
		&core.TextField{Name: "razorpay_payment_id", Max: 64},
	...
	collection.AddIndex("idx_rzp_live_event_id", true, "event_id", "")
	collection.AddIndex("idx_rzp_live_event_order", false, "live_order,received_at", "live_order != ''")
	collection.AddIndex("idx_rzp_live_event_type", false, "event_type,received_at", "")
	if err := app.Save(collection); err != nil {
		return nil, err
	}
	return collection, nil
